    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch(ev.getAction()){
            case MotionEvent.ACTION_DOWN:
                 ...
                 * 参数1：  Runnable接口,要执行的内容
                 * 参数2：  延迟时间delayMillis(单位：ms)后执行Runnable接口的内容
                 */
                mHandler.postDelayed(mLongClickRunnable, dragResponseMS);
                mDownX = (int) ev.getX();
                mDownY = (int) ev.getY();
                //根据按下的X,Y坐标获取所点击item的position，即依据触摸点的X,Y坐标计算出点击的是GridVIew的哪个Item
                mDragPosition = pointToPosition(mDownX, mDownY);
                /**
                 * INVALID_POSITION:代表无效的位置。
                 * AdapterView:
                 *   1.作用:以列表的形式显示数据。
